...
# EfficientAttention for 1D sequence (适配Transformer输入)
class EfficientAttention(nn.Module):
    ...
    def forward(self, input_):
        # input_: (batch, seq_len, in_channels)
        n, seq_len, _ = input_.size()
        keys = self.keys(input_).transpose(1, 2)  # (batch, key_channels, seq_len)
        queries = self.queries(input_).transpose(1, 2)  # (batch, key_channels, seq_len)
        values = self.values(input_).transpose(1, 2)  # (batch, value_channels, seq_len)
        ...
        for i in range(self.head_count):
            ...
            attended_value = (context.transpose(1, 2) @ query).transpose(1, 2)
            attended_values.append(attended_value)
        aggregated_values = torch.cat(attended_values, dim=2)  # (batch, seq_len, value_channels)
...
class CrossBlock(nn.Module):
    def __init__(
        ...
        # EfficientAttention参数设置
        self.efficient_attn0 = EfficientAttention(embed_dim, inner_dim, num_heads, inner_dim)
        self.efficient_attn1 = EfficientAttention(embed_dim, inner_dim, num_heads, inner_dim)
    ...
    def forward(
        ...
        # 用 EfficientAttention 替换原交叉注意力
        m0 = self.efficient_attn0(x0)
        m1 = self.efficient_attn1(x1)
...
# Device: cuda
# Total parameters: 17,772,917
# Trainable parameters: 17,772,917
# Frozen parameters: 0
# Parameters by top-level module:
#   transformers         17,176,356
